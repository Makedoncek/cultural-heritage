...
export default function VisitImpressionModal({visit, onClose, onUpdated}: Readonly<Props>) {
    ...
    const handleSave = async () => {
        if (futureDate) {
            toast.error(t('passport.modal.futureDateError'));
            return;
        }
        setSaving(true);
        try {
            ...
            onUpdated(updated);
            toast.success(t('passport.modal.saved'));
            onClose();
        } catch {
            toast.error(t('passport.modal.saveError'));
        } finally {
            setSaving(false);
        }
    };
            ...
            <div className="bg-white dark:bg-stone-900 border border-gray-200 dark:border-stone-700 rounded-2xl shadow-xl max-w-md w-full p-6">
                ...
                {futureDate && (
                    <p className="text-red-600 dark:text-red-400 text-xs mb-3">
                        {t('passport.modal.futureDateError')}
                    </p>
                )}
                {!futureDate && <div className="mb-3"/>}

                <label className="block text-sm font-medium text-gray-700 dark:text-stone-200 mb-1">
                    {t('passport.modal.impressionLabel')}
                </label>
                <textarea
                    value={impression}
                    ...
                    maxLength={2000}
                    rows={5}
                    placeholder={t('passport.modal.impressionPlaceholder')}
                ...
                <label className="flex items-center gap-2 mb-4 cursor-pointer text-sm text-gray-700 dark:text-stone-200">
                    ...
                    />
                    {isPublic
                        ? <span><strong>🌐 {t('passport.modal.public')}</strong> {t('passport.modal.publicHint')}</span>
                        : <span><strong>🔒 {t('passport.modal.private')}</strong> {t('passport.modal.privateHint')}</span>}
                </label>
                ...
                <div className="flex gap-2 justify-end">
                    ...
                    >
                        {saving ? t('passport.modal.saving') : t('passport.modal.save')}
                    </button>
                </div>
            </div>
        </div>
